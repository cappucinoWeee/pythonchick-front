...
export const AppProvider = ({ children }) => {
  ...
  const fetchCourses = async (userId = user?.id) => {
    ...
    try {
      ...
      const processedCourses = Array.isArray(data) ? data.map(processCourseData) : [];
  ...
  const processCourseData = (course) => {
    ...
    const processedTopics = course.topics
      ? course.topics.map((topic) => {
          // Process lessons if they exist
          const processedLessons = topic.lessons
  ...
  const completeLesson = async (courseId, topicId, lessonId, score = null) => {
    try {
      ...
      setCourses((prevCourses) => {
        ...
        // Find the topic
        const topicIndex = newCourses[courseIndex].topics.findIndex(
