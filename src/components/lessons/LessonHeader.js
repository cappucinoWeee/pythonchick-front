...
const LessonHeader = ({ course, topic, lesson, lessonIndex = 0, topicLessonsCount = 0 }) => {
  ...
  const progressPercent = Math.round(((lessonIndex + (isCompleted ? 1 : 0)) / Math.max(topicLessonsCount, 1)) * 100);
    ...
    <div className="bg-white rounded-xl shadow-card mb-6 p-6">
      <div className="flex items-center mb-2">
        <Button 
          ...
          className="mr-2"
        />
        <Title level={3} className="m-0">{lessonTitle}</Title>
      </div>
      
      <div className="flex items-center mb-4">
        <Text className="mr-4">Lesson {lessonIndex + 1}/{Math.max(topicLessonsCount, 1)}</Text>
