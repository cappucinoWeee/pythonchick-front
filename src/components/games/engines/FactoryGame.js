...
const FactoryGame = ({ gameData, onComplete, onScoreUpdate, userProgress }) => {
      ...
      <Card className="shadow-md border-0 text-center py-8">
        ...
        >
          ...
          <Paragraph className="mb-4">
            Congratulations! You've successfully built a working factory using Python functions and
            earned {score} XP!
          </Paragraph>
  ...
  const completedTasks =
    factoryLevels.slice(0, currentLevel).reduce((sum, level) => sum + level.tasks.length, 0) +
    currentTask;
    ...
    <div className="factory-game">
        ...
        <Row gutter={[16, 16]} align="middle">
          <Col span={16}>
            <Title level={4} className="mb-0">
              Factory Production: Level {currentLevel + 1}
            </Title>
            <Progress
      ...
      <Card className="shadow-md border-0">
        <Title level={4}>
          {task.title}
          <Badge
            count={`Task ${currentTask + 1}/${levelData.tasks.length}`}
        ...
        {showHint && (
          <Alert
            ...
            description={
              task.hint ||
              'Try using function definitions and proper syntax to solve this challenge.'
            }
            type="info"
            showIcon
            icon={<BulbOutlined />}
