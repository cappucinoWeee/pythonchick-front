...
const TopicProgress = ({ topic, course }) => {
  ...
  if (!topic || !topic.lessons || !Array.isArray(topic.lessons)) {
    return (
      <Card className="shadow-md border-0">
        <div className="text-center py-8 text-gray-500">No lessons available for this topic.</div>
      </Card>
    );
  }
              ...
              <Col key={lesson.id} span={12}>
                <Link
                  to={
                    isLocked
                      ? '#'
                      : `/courses/${course?.id}/topics/${topic.id}/lessons/${lesson.id}`
                  }
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex-grow min-w-0">
                      ...
                      <Text className={`text-xs ${isLocked ? 'text-gray-400' : 'text-gray-500'}`}>
                        {lesson.type === 'quiz'
                          ? 'Quiz'
                          : lesson.type === 'coding'
                            ? 'Coding'
