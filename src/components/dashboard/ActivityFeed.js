...
const ActivityCard = ({ activity }) => {
  return (
    <div className="flex p-3 border-b last:border-0 hover:bg-gray-50 transition-colors">
      <div className={`flex-shrink-0 mr-3 w-10 h-10 rounded-full bg-${activity.iconColor}-100 flex items-center justify-center`}>
        {activity.icon}
      </div>
          ...
          <div>
            ...
            {activity.course && (
              <Link to={`/courses/${activity.courseId}`}>
                <Tag color="blue" className="mr-0">{activity.course}</Tag>
              </Link>
            )}
          </div>
